...
class VisualCrossingTamb:
    ...
    def get_visual_crossing_raw_data(self, latitude, longitude, start_date, end_date):
        ...
        response = requests.get(self.base_url + '/{},{}/{}/{}?unitGroup=metric&key={}&include=hours&elements=datetime,temp'.format(
    ...
    def get_specific_data(self, all_data):
        ...
        final = all_data[['temp']].copy()
        final.rename(columns={'temp': 'Tamb'}, inplace=True)
        return final
    ...
    def get_tamb_from_visualcrossing(self, plant_id, start_date, end_date, freq=None):
        ...
        vc_tamb = self.get_visual_crossing_raw_data(latitude, longitude, start_dt, end_dt)

        # resample at given frequency using linear interpolation
        if freq:
            vc_tamb = vc_tamb.resample(freq).mean().interpolate()
